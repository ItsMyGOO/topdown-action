...
public class PhantomCamera2D : PhantomCamera
{
    ...
    {
        get =>
            Node2D.Call(PhantomCamera.MethodName.GetFollowTargets).AsGodotArray<Node2D>().ToArray();
        set => Node2D.Call(PhantomCamera.MethodName.SetFollowTargets, new Array<Node2D>(value));
    }

    public void AppendFollowTargets(Node2D target) =>
        Node2D.Call(PhantomCamera.MethodName.AppendFollowTargets, target);

    public void AppendFollowTargetsArray(Node2D[] targets) =>
        Node2D.Call(PhantomCamera.MethodName.AppendFollowTargetsArray, targets);

    public void EraseFollowTargets(Node2D target) =>
        Node2D.Call(PhantomCamera.MethodName.EraseFollowTargets, target);

    public FollowMode2D FollowMode =>
        (FollowMode2D)(int)Node.Call(PhantomCamera.MethodName.GetFollowMode);
    ...
    public void EmitNoise(Transform2D transform) =>
        Node2D.Call(PhantomCamera.MethodName.EmitNoise, transform);
    ...
    public PhantomCamera2D(GodotObject phantomCameraNode)
        : base(phantomCameraNode)
    {
        var callableTweenInterrupted = Callable.From<Node2D>(pCam =>
            TweenInterrupted?.Invoke(pCam)
        );
        var callableDeadZoneReached = Callable.From(
            (Vector2 side) => DeadZoneReached?.Invoke(side)
        );
        var callableNoiseEmitted = Callable.From(
            (Transform2D output) => NoiseEmitted?.Invoke(output)
        );
    ...
    public static new class MethodName
    {
        ...
        public static readonly StringName GetRotationDampingValue = new(
            "get_rotation_damping_value"
        );
        public static readonly StringName SetRotationDampingValue = new(
            "set_rotation_damping_value"
        );
        ...
        public static readonly StringName GetLookaheadAcceleration = new(
            "get_lookahead_acceleration"
        );
        public static readonly StringName SetLookaheadAcceleration = new(
            "set_lookahead_acceleration"
        );

        public static readonly StringName GetLookaheadDeceleration = new(
            "get_lookahead_deceleration"
        );
        public static readonly StringName SetLookaheadDeceleration = new(
            "set_lookahead_deceleration"
        );
